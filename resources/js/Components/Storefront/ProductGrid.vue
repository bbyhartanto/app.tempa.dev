...
<template>
    <div>
        ...
        <!-- Products Grid -->
        <div v-else :class="['grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4', isGlass ? 'glassify p-5 rounded-2xl border-none' : '']">
            <ProductCard
                v-for="product in products"
                :key="product.id"
                :product="product"
                :store-link="storeLink"
                :show-price="templateConfig.layout?.show_prices !== false"
                :primary-color="templateConfig.colors?.primary"
